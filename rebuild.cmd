...
IF NOT EXIST "%CMAKECMD%" (
    ...
    echo ----------------------------------
    echo  Rebuilding of jsoncreator %MODE% mode
    echo ----------------------------------
)
...
set "FILENAME=%PROJECTDIR%buildWin%MODE%\srcjsoncreator\%MODE%\jsoncreator.exe"

IF EXIST "%FILENAME%" (
  echo   jsoncreator.exe built for %MODE% mode is here:
  echo %FILENAME%
) ELSE (
  echo   jsoncreator.exe built for %MODE% mode could not be found at
  echo %FILENAME%
)
